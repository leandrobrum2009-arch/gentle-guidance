  ...
  return (
    <AdminLayout>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold text-white tracking-tight">Banners Promocionais</h1>
          <p className="text-slate-400 mt-1">Gerencie os banners do carrossel da página inicial.</p>
        </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {isLoading ? (
          ...
          <Card key={banner.id} className="border-white/5 bg-[#0d0d0f]/50 backdrop-blur-xl overflow-hidden group">
            <div className="aspect-[21/9] w-full relative overflow-hidden bg-slate-900">
              ...
              <div className="absolute top-2 right-2">
                <Badge className={banner.is_active ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/20" : "bg-slate-500/20 text-slate-400 border-slate-500/20"}>
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-bold text-white tracking-tight">{banner.title}</h3>
                  <p className="text-xs text-slate-400 mt-1 line-clamp-1">{banner.subtitle || "Sem subtítulo"}</p>
                </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Ordem: {banner.order_index}</span>
              </div>
            </CardContent>
          </Card>
        ))}
        {!isLoading && banners?.length === 0 && (
          <div className="col-span-full py-20 text-center border-2 border-dashed border-white/5 rounded-3xl">
            ...
            <p className="text-slate-500 font-medium font-display">Nenhum banner cadastrado ainda.</p>
